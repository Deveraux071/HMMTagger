    ...
    for f in file_list:
        with open(f) as file:
            ...
            for line in file:
                ...
                #                          'CJS-AVQ', 'PRP-CJS', 'DT0-CJT', 'PNI-CRD', 'NP0-NN1', 'VVB-NN1',
                #                          'VVG-NN1', 'VVZ-NN2', 'VVN-VVD']:
                #         alt_tag_to_append = tag_to_append[-3:] + '-' + tag_to_append[:3]
                #         if alt_tag_to_append not in distinct_tags:
                #             distinct_tags.append(alt_tag_to_append)
                #     else:
                #         distinct_tags.append(tag_to_append)
                    # if '-' in tag_to_append:
                    #     alt_tag_to_append = tag_to_append[-3:] + '-' + tag_to_append[:3]
                ...
                if word_to_append in ['.', '?', '!']:
                    ...
                    sentence_flag = 1
        # if len(tags_list) != len(sentences):
        #     print("Something is wrong")
        # for i in range(0, len(sentences)):
        #     if len(sentences[i]) != len(tags_list[i]):
        #         print("Something is wrong")
        # print("DW everything is G")
        # print("Sentences:\n")
        # print(sentences)
        # print("Tags:\n")
        # print(tags_list)
...
def initial_probability_table_generator(sentences, tags):
    ...
    for i in range(0, len(sentences)):
        ...
        # Converting ambiguous tags into the proper order if required:
        # if first_tag in ['AV0-AJ0', 'VVN-AJ0', 'VVD-AJ0', 'NN1-AJ0', 'VVG-AJ0', 'PRP-AVP',
        #                  'CJS-AVQ', 'PRP-CJS', 'DT0-CJT', 'PNI-CRD', 'NP0-NN1', 'VVB-NN1',
        #                  'VVG-NN1', 'VVZ-NN2', 'VVN-VVD']:
        #     first_tag = first_tag[-3:] + '-' + first_tag[:3]
    ...
    print(initial_probability_table)
    print(len(initial_probability_table))
...
def emission_probability_table_generator(sentences, tags):
    ...
    for i in range(0, len(sentences)):
        ...
        for j in range(0, len(sentence)):
            ...
            # Converting ambiguous tags into the proper order if required:
            # if word_tag in ['AV0-AJ0', 'VVN-AJ0', 'VVD-AJ0', 'NN1-AJ0', 'VVG-AJ0', 'PRP-AVP',
            #                 'CJS-AVQ', 'PRP-CJS', 'DT0-CJT', 'PNI-CRD', 'NP0-NN1', 'VVB-NN1',
            #                 'VVG-NN1', 'VVZ-NN2', 'VVN-VVD']:
            #     word_tag = word_tag[-3:] + '-' + word_tag[:3]
    ...
    # Covering observation counts to probability
...
def viterbi_calculator(sentences, distinct_tags, initial_table, transition_table, emission_table):
    ...
    for i in range(0, len(sentences)):
        ...
        for j in range(0, len(distinct_tags)):
            ...
            # [DONE] TODO: Observed word may not exist in the trained words. Handling of such cases is pending.
            #  RN assuming that all observed words exist in training set [DONE]
        ...
        for k in range(1, len(sentence)):
            ...
            for j in range(0, len(distinct_tags)):
                ...
                for ind in range(0, len(prob[k-1])):
                    ...
                    # [NON-PRIORITY] TODO: previous state and current state may not always have a transitional relationship.
                    #  Currently assuming they do. [NON-PRIORITY]
                ...
                # for prev_prob in prob[k-1]:
                #     transitions_second_tags = transition_table[cur_tag]  # All transitions with cur_tag as the first tag
                #     # TODO: previous state and current state may not always have a transitional relationship.
                #     #  Currently assuming they do.
                #     if cur_tag in transitions_second_tags:
                #         to_append = prev_prob * transitions_second_tags[cur_tag]
                #     else:
                #         to_append = 0
                #     prev_transition_product_list.append(to_append)
                ...
                #  [DONE] TODO: This assumes that given we know the tag (i.e. the state), we know the probability of
                #  the word in this iteration (i.e. the observation).
                #  We might not have the emission-state relationship in training. Need to find a way to account for that.
                #  Currently assigning 0 prob to emission-state relationships seen in test but not in training. [DONE]
    ...
    # for j in range(0, len(sentence)):
    #     word = sentence[j]
    #     if word in initial_table:
    #         if word in emission_table[]
    #         prob[0, j] = initial_table[j] * emission_table[]
...
def transition_probability_table_generator(sentences, tags):
    ...
    for i in transition_probability_table:
        print(sum(transition_probability_table[i].values()))
...
def tag(training_list, test_file, output_file):
    ...
    #
    #  [DONE] TODO: Does not account for multiple files in training_list. Currently assumes that training list is only one file. [DONE]
    ...
    # actual_tags = []
    # for i in tags:
    #     actual_tags.extend(i)
    # wrong = 0
    # for i in range(0, len(tags)):
    #     tags_sub = tags[i]
    #     predicted_tags_sub = predicted_tags[i]
    #     for bun in range(0, len(tags_sub)):
    #         if tags_sub[bun] != predicted_tags_sub[bun]:
    #             wrong += 1
    # # for bun in range(0, len(actual_tags)):
    # #     if actual_tags[bun] != predicted_tags[bun]:
    # #         wrong += 1
    # right = len(actual_tags) - wrong
    # accuracy = right/len(actual_tags)
...
if __name__ == '__main__':
    ...
    # tag(['data/training2.txt'], 'data/test1.txt', 'output_1.txt')
